...
public class BottomSheetBehavior2<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private boolean needHeightChangeAnimation;//高度变化是否需要动画，仅一次有效
    private View curPanelView;//当前面板的view

    public void setCurPanelView(View curPanelView) {
        this.curPanelView = curPanelView;
    }
    ...
    public void setNeedHeightChangeAnimation(boolean needHeightChangeAnimation) {
        this.needHeightChangeAnimation = needHeightChangeAnimation;
    }
    ...
    public BottomSheetBehavior2(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, com.google.android.material.R.styleable.BottomSheetBehavior_Layout);
        this.shapeThemingEnabled = a.hasValue(com.google.android.material.R.styleable.BottomSheetBehavior_Layout_shapeAppearance);
        boolean hasBackgroundTint = a.hasValue(com.google.android.material.R.styleable.BottomSheetBehavior_Layout_backgroundTint);
        if (hasBackgroundTint) {
            ColorStateList bottomSheetColor =
                    MaterialResources.getColorStateList(
                            context, a, com.google.android.material.R.styleable.BottomSheetBehavior_Layout_backgroundTint);
        ...
        setSaveFlags(a.getInt(com.google.android.material.R.styleable.BottomSheetBehavior_Layout_behavior_saveFlags, SAVE_NONE));
        setHalfExpandedRatio(a.getFloat(com.google.android.material.R.styleable.BottomSheetBehavior_Layout_behavior_halfExpandedRatio, 0.5f));
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, final V child, int layoutDirection) {
        ...
        if (viewRef == null) {
            ...
            peekHeightMin =
                    parent.getResources().getDimensionPixelSize(com.google.android.material.R.dimen.design_bottom_sheet_peek_height_min);
        ...
        //TODO
        final int preFitToContentsOffset = fitToContentsOffset;
...
//            ViewCompat.offsetTopAndBottom(child, getExpandedOffset());
            //高度变化动画
            if (needHeightChangeAnimation) {
                needHeightChangeAnimation = false;
                if (preFitToContentsOffset != fitToContentsOffset) {
                    ViewCompat.offsetTopAndBottom(child, preFitToContentsOffset);
                    //延时动画，防止面板切换过渡的时候进行动画了
                    child.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            ValueAnimator valueAnimator = ValueAnimator.ofInt(preFitToContentsOffset, fitToContentsOffset);
                            valueAnimator.addUpdateListener(new AnimatorUpdateListener() {
                                @Override
                                public void onAnimationUpdate(ValueAnimator animation) {
                                    ViewCompat.offsetTopAndBottom(child, (Integer) animation.getAnimatedValue() - child.getTop());
                                }
                            });
                    ...
                    ViewCompat.offsetTopAndBottom(child, fitToContentsOffset);
                }
            } else {
                ViewCompat.offsetTopAndBottom(child, fitToContentsOffset);
            }
        ...
        //TODO
        View scrollingParent = child;
        if (curPanelView != null) {
            scrollingParent = curPanelView;
        }
        nestedScrollingChildRef = new WeakReference<>(findScrollingChild(scrollingParent));
        return true;
    }
    ...
    public boolean onStartNestedScroll(
        ...
        //TODO
        return draggable && (axes & ViewCompat.SCROLL_AXIS_VERTICAL) != 0;
    }
    ...
    public void onStopNestedScroll(
        ...
        } else if (hideable && shouldHide(child, yVel)) {//TODO
            top = parentHeight;
        ...
        //TODO
        if (yVel < 1200) {//如果速度没有达到fling，则根据移动距离（面板高度的1/5）判断展开还是关闭
            if (child.getTop() - (parentHeight - child.getHeight()) > child.getHeight() / 5) {
                top = collapsedOffset;
                targetState = STATE_COLLAPSED;
            } else {
                top = fitToContentsOffset;
    ...
    private void createMaterialShapeDrawable(
        ...
        if (this.shapeThemingEnabled) {
            this.shapeAppearanceModelDefault =
                    new ShapeAppearanceModel(context, attrs, com.google.android.material.R.attr.bottomSheetStyle, DEF_STYLE_RES);
    ...
    private final ViewDragHelper.Callback dragCallback =
            new ViewDragHelper.Callback() {
                ...
                public boolean tryCaptureView(@NonNull View child, int pointerId) {
                    ...
                    return draggable && viewRef != null && viewRef.get() == child;
                }
                ...
                public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
                    ...
                    //TODO
                    if (yvel == 0.0f) {//如果速度没有达到fling，则根据移动距离（面板高度的1/5）判断展开还是关闭
                        if (releasedChild.getTop() - (parentHeight - releasedChild.getHeight()) > releasedChild.getHeight() / 5) {
                            top = collapsedOffset;
                            targetState = STATE_COLLAPSED;
                        } else {
                            top = fitToContentsOffset;
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcelable superState, BottomSheetBehavior2 behavior) {
            super(superState);
    ...
    public static <V extends View> BottomSheetBehavior2<V> from(V view) {
